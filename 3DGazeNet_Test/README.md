...
- **보정**: 전체화면 타겟을 하나씩 응시하며 `SPACE`. 끝나면 시선→화면 좌표 **ridge 정규화 회귀** 적합
  (One-Euro 필터로 지터 억제, 머리이동 보정 특징). 점이 많을수록 정확(`--points 5`).
- **자동 강건화**: 정규화 세기 **λ는 LOO-CV가 최소가 되는 값으로 자동선택**(`--ridge-lam`에 숫자를 주면 고정),
  **잘못 캡처된 이상치 보정점은 자동 제거** 후 재적합. 보정 끝나면 **LOO-CV(px·도)·축별 x/y 오차·yaw/pitch
  신호범위**를 출력하고, 수직 신호가 약하거나(`수직 신호 약함`) 정확도가 나쁘면(`머리 움직임 의심`) 경고한다.
